...
import { Trophy, Calendar, ChevronRight, X, ExternalLink, ShieldCheck, ArrowRight, Swords, Users } from 'lucide-react';
...
export default function PastTournaments({ limit }: PastTournamentsProps) {
  ...
  async function fetchTournaments() {
    try {
      ...
      if (data) {
        ...
        // Sort nearest/most recent event date to today first!
  ...
  return (
    <div className="my-8 sm:my-16">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-6 sm:mb-8">
        ...
        {/* Filter Buttons */}
        <div className="flex items-center gap-1 bg-[#161512] p-1 rounded-lg border border-chess-border self-start sm:self-auto overflow-x-auto max-w-full">
          {(['ALL', 'OTB', 'LICHESS', 'CHALLENGE'] as const).map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`px-2.5 sm:px-3 py-1.5 rounded-md text-xs font-bold transition-colors cursor-pointer whitespace-nowrap ${
                filter === f
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
            {(limit ? filteredTournaments.slice(0, limit) : filteredTournaments).map((t) => (
              <div
                key={t.name}
                onClick={() => setSelectedTournament(t)}
                className="bg-surface hover:bg-[#2E2B27] border border-chess-border hover:border-primary/50 p-4 sm:p-5 rounded-xl transition-all duration-150 cursor-pointer group flex flex-col justify-between shadow-card active:scale-[0.99]"
              >
                <div>
                  ...
                  <h3 className="text-base sm:text-lg font-bold text-white mb-2 group-hover:text-primary transition-colors line-clamp-2">
                    {t.name}
                  </h3>

                  <div className="grid grid-cols-2 gap-2 my-3 sm:my-4 p-2.5 sm:p-3 rounded-lg bg-[#161512] border border-chess-border text-xs">
                    <div>
                      <span className="text-text-muted text-[10px] uppercase block flex items-center gap-1">
                        <Swords className="w-3 h-3 text-primary shrink-0" /> Matches
                      </span>
                      <span className="font-extrabold text-white text-xs sm:text-sm">{t.gameCount} Games</span>
                    </div>
                    <div>
                      <span className="text-text-muted text-[10px] uppercase block flex items-center gap-1">
                        <Users className="w-3 h-3 text-amber-400 shrink-0" /> Players
                      </span>
                      <span className="font-extrabold text-white text-xs sm:text-sm">{t.uniquePlayerCount} Players</span>
                    </div>
                  </div>
                </div>

                <div className="pt-3 border-t border-chess-border/60 flex items-center justify-between text-xs">
                  <span className="text-text-muted text-[11px] truncate max-w-[170px] flex items-center gap-1">
                    <Trophy className="w-3 h-3 text-yellow-500 shrink-0" />
                    <span>{t.topScorerName ? `Top: ${t.topScorerName}` : 'FCA Rated Event'}</span>
                  </span>
          ...
          {limit && filteredTournaments.length > limit && (
            <div className="text-center mt-6">
              <Link
                to="/tournaments"
                className="inline-flex items-center gap-2 text-primary hover:text-primary-light font-bold text-xs sm:text-sm hover:underline transition-all cursor-pointer group"
              >
          ...
          <div className="bg-surface border border-chess-border rounded-xl w-full max-w-2xl max-h-[85vh] flex flex-col shadow-2xl overflow-hidden">
            ...
            <div className="p-4 sm:p-5 border-b border-chess-border flex items-start justify-between gap-3 bg-[#161512]">
              <div>
                ...
                <h2 className="text-lg sm:text-xl font-bold text-white leading-tight">{selectedTournament.name}</h2>
              ...
              <button
                ...
                className="p-1 rounded-lg bg-surface text-text-muted hover:text-white border border-chess-border cursor-pointer shrink-0"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-4 sm:p-5 overflow-y-auto space-y-3 divide-y divide-chess-border/40">
                  ...
                  <div key={game.id} className="pt-3 first:pt-0 flex items-center justify-between gap-3 text-xs">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 mb-1 flex-wrap">
                        <span className="font-bold text-white truncate max-w-[120px] sm:max-w-none">{game.white_player?.full_name || 'White'}</span>
                        <span className="text-text-muted text-[10px]">vs</span>
                        <span className="font-bold text-white truncate max-w-[120px] sm:max-w-none">{game.black_player?.full_name || 'Black'}</span>
                      </div>
